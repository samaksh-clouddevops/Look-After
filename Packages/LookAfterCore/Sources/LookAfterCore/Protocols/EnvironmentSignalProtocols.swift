...
/// Health-derived slice of `EnvironmentContext` (no HealthKit imports in LookAfterCore).
...
/// Reads health context from already-fetched summaries (HealthKit stays in LookAfterHealth / app layer).
...
/// Weather slice of `EnvironmentContext`. WeatherKit implementations live outside LookAfterCore.
