...
final class TourPlacementEngineTests: XCTestCase {
    ...
    func testArrowPointsTowardHighlight() {
        ...
        // Tip should lie on the card edge facing the highlight.
    ...
    func testSmallPhoneSELayout() {
        ...
        XCTAssertLessThan(
            proposal.cardFrame.intersectionArea(with: padded.expanded(by: 12)),
            0.5
        )
    }
    ...
    /// Visual-regression style fixture: fixed frames must stay stable across refactors.
    func testPlacementSnapshotFixtureStable() {
        ...
        // Card fully below padded highlight and inside usable area
        let padded = highlight.insetBy(dx: -10, dy: -10)
