...
public final class DecideForMePicker: @unchecked Sendable {
    ...
    public func pick(
        ...
        let prompt = LifeOSPrompts.decideForMePrompt(snapshot: snapshot, tasks: candidates)
        do {
            let raw = try await glm.complete(prompt: prompt, systemPrompt: LifeOSPrompts.structuredOutputSystem)
